...
namespace WAPIWrapperCSharp
{
    ...
    public delegate void WindCallback(ulong reqId, WindData wd);
    public delegate void TradeCallback(ulong LogonID, WindData wd);
    ...
    public class WindAPI
    {
        ...
        Dictionary<ulong, TradeCallback> TradeCallbackList = new Dictionary<ulong, TradeCallback>();
        //构造函数
        public WindAPI()
        {
            try
        ...
        //多值函数bbq，获得实时行情，不带WindCallback传参为取快照
        //带WindCallback传参为订阅，数据通过fnCallback返回
        public WindData bbq(string windCodes, string fields, string options)
        {
            checkConnection();
            object ocodes, ofields, otimes, odata;
            int errCode;
            odata = wdc.bbq_syn(windCodes, fields, options, out ocodes, out ofields, out otimes, out errCode);
            return formatWD(ocodes, ofields, otimes, odata, errCode);
        }
        ...
        /// 第三方开发App进行权限认证
        /// </summary>
        /// <param name="appKey">第三方开发的app从大奖章网站获取唯一认证的Id</param>
        /// <param name="appSecret">第三方开发app的秘钥</param>
        /// <param name="options">保留参数</param>
        /// <returns>认证数据</returns>
        public WindData wappAuth(string appKey, string appSecret, string options) {
            checkConnection();
            object ofields, odata, ocodes, otimes;
            int errCode;
            odata = wdc.wappAuth_syn(appKey, appSecret, options, out ocodes, out ofields, out otimes, out errCode);
            return formatWD(ocodes, ofields, otimes, odata, errCode);
        }
        ...
        /// 第三方开发App和大奖章进行通讯函数
        /// </summary>
        /// <param name="type_id">communication type,include:message_put; message_get; app_version; app_fps; app_data_get; app_data_put</param>
        /// <param name="message">communication content</param>
        /// <param name="options">保留参数</param>
        /// <returns>通讯结果</returns>
        public WindData wappMessage(string type_id, string message, string options)
        {
            checkConnection();
            object ofields, odata, ocodes, otimes;
            int errCode;
            odata = wdc.wappMessage_syn(type_id, message, options, out ocodes, out ofields, out otimes, out errCode);
            return formatWD(ocodes, ofields, otimes, odata, errCode);
        }
        ...
        private WindData formatWD(object ocodes, object ofields, object otimes, object odata, int errorCode)
        {
            ...
            if (otimes != null)
            {
                ...
                for (int i = 0; i < length; i++)
                {
                    wd.timeList[i] = DateTime.FromOADate(tmpTimes[i] - 693960);
                }
            }
            wd.data = odata;
            return wd;
